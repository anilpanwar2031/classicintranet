    ...
    success: function(data)
    {
       ...
       console.log("in Product ajax success");
        ...
        for (let i = 0; i < data.length; i++)
        {
         ...
         output += "<div class='card'><div class='row'><div class='col-sm-12'><img class='product-img' src={% static 'dist/img/image_77.png' %}" +
                  "alt='Card image' /><div class='card-body-right'><p class='sidebar-heading mb-0'>"+
                   {{p.name}} + " <span class='active-button'>Active</span></p>"+
                                "<p class='sidebar-description mb-0'><span class='card-text'>" + {{p.description}} +"</span></p>"+
                                "<p class='sidebar-footer-text mb-0'>" + {{p.selling_price}}+ "</p></div></div></div></div>";
